...
async function runMasterSimulation() {
    ...
    const duplicateRequests = Array.from({ length: 10 }, () => axios_1.default
    ...
    for (let i = 0; i < 5; i++) {
        await axios_1.default.post(`${API_BASE}/checkout/simulate-drop`, { rail: 'netbanking_SBI' });
    }
    const breakerCheck = await axios_1.default.get(`${API_BASE}/checkout/health-check`);
    ...
    await axios_1.default.post(`${API_BASE}/webhooks/razorpay`, signed.payload, {
    ...
    const reconcileRes = await axios_1.default.post(`${API_BASE}/webhooks/razorpay`, signed.payload, {
        headers: { 'Content-Type': 'application/json', 'X-Razorpay-Signature': signed.signature },
    });
    ...
    const ptpRes = await axios_1.default.post(`${API_BASE}/invoices/negotiate-ptp`, {
        invoiceId: invId,
    ...
    // ---------------------------------------------------------------------------
    const finalMetrics = await axios_1.default.get(`${API_BASE}/metrics`);
    ...
    console.log(`Gross Revenue at Risk: ${(0, razorpay_1.formatRazorpayAmount)((kpis.grossRevenueAtRisk || 0) * 100, 'INR')}`);
