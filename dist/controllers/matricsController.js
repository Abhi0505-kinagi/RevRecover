...
const getSystemMetrics = async (_req, res) => {
    try {
        ...
        const ledgerStats = await RecoveryLedger_1.RecoveryLedger.aggregate([
            {
                $group: {
                    _id: null,
        ...
        const [routeAWaiting, routeBWaiting] = await Promise.all([
            recoveryQueues_1.routeAQueue.getWaitingCount(),
            recoveryQueues_1.routeBQueue.getWaitingCount(),
        ]);
        const railStatuses = await circuitBreaker_1.CircuitBreakerService.getCheckoutRailStatus();
        const invoiceStats = await Invoice_1.Invoice.aggregate([
