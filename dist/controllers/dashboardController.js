...
async function handleSeedData(req, res) {
    try {
        await RecoveryLedger_1.RecoveryLedger.deleteMany({});
        await Invoice_1.Invoice.deleteMany({});
        await RecoveryLedger_1.RecoveryLedger.insertMany([
            {
        ...
        await Invoice_1.Invoice.insertMany([
            {
...
async function renderDashboard(req, res) {
    try {
        const b2cCount = await RecoveryLedger_1.RecoveryLedger.countDocuments();
        const b2bCount = await Invoice_1.Invoice.countDocuments();
        console.log(`📊 [DASHBOARD QUERY] DB: "${mongoose_1.default.connection.name}" | RecoveryLedger: ${b2cCount} | Invoices: ${b2bCount}`);
        const ledgerAgg = await RecoveryLedger_1.RecoveryLedger.aggregate([
            {
                $group: {
                    _id: null,
        ...
        const invoiceAgg = await Invoice_1.Invoice.aggregate([
            {
                $group: {
                    _id: null,
        ...
        const aor = addressableAtRisk > 0
            ...
            : grossRecoveryRate;
        const recentLedgers = await RecoveryLedger_1.RecoveryLedger.find().sort({ _id: -1 }).limit(10).lean();
        const formatINR = (paise) => `₹${((paise || 0) / 100).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`;
        const html = `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="flex justify-between items-center pb-6 border-b border-slate-800">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400">Database: <span class="text-emerald-400 font-mono">${mongoose_1.default.connection.name || 'default'}</span></p>
        </div>
      </div>
      <div class="flex items-center gap-3">
        ...
        <div class="divide-y divide-slate-800 text-xs">
          ${recentLedgers.length === 0
            ...
            <div class="py-3 flex justify-between items-center">
              <div>
                <span class="px-2 py-0.5 rounded font-mono font-bold ${l.status === 'RECOVERED'
                ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                : l.status === 'TERMINAL_DLQ'
                    ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20'
                    : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}">${l.status}</span>
