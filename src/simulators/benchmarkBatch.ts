...
const RUN_LABEL = (process.env.RUN_LABEL || 'engine') as 'control' | 'engine';
...
interface B2CCase {
  ...
  route?: string;
  debitedRisk: boolean;
  lateAuthScheduled: boolean;
  linkCompletionScheduled: boolean;
}

async function postWebhook(payload: any) {
  return axios.post(`${API_BASE}/webhooks/razorpay`, payload, {
    headers: { 'Content-Type': 'application/json', 'X-Razorpay-Signature': sign(payload) },
  });
}

async function injectB2CFailure(index: number): Promise<B2CCase> {
  const err = pickWeighted();
  const amountPaise = Math.floor(50000 + Math.random() * 950000);
  const paymentId = `pay_bench_${RUN_LABEL}_${Date.now()}_${index}_${crypto.randomBytes(3).toString('hex')}`;
...
async function fireFollowUpCapture(paymentId: string, amountPaise: number) {
...
interface B2BInvoice {
  invoiceId: string;
  amountPaise: number;
  ptpReplyScheduled: boolean;
  ptpHonoured: boolean;
}

async function injectB2BInvoice(index: number): Promise<B2BInvoice> {
...
async function sendPtpReply(inv: B2BInvoice) {
  const dateStr = new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  try {
    await axios.post(`${API_BASE}/invoices/negotiate-ptp`, {
...
async function firePaymentLinkCompletion(invoiceId: string, amountPaise: number) {
  const payload = {
    entity: 'event', account_id: 'acc_benchmark', event: 'payment.captured', contains: ['payment'],
    payload: { payment: { entity: {
...
async function run() {
  ...
  const b2cTerminal = new Map<string, any>();
  await pollUntilTerminal(async () => {
    let allDone = true;
    for (const c of b2cCases) {
      if (b2cTerminal.has(c.paymentId)) continue;
  ...
  for (const inv of b2bInvoices) {
    ...
    if (!doc) { b2bOtherCount++; continue; }
    if (doc.status === 'PAID') { b2bPaidCount++; b2bPaidPaise += inv.amountPaise; }
    else if (doc.status === 'ESCALATED_LEGAL') b2bEscalatedCount++;
    else if (doc.status === 'PTP_BROKEN') b2bBrokenCount++;
    else b2bOtherCount++;
  }
  ...
  for (const [route, s] of Object.entries(byRoute)) console.log(`${route}: ${s.recovered}/${s.total} recovered`);
