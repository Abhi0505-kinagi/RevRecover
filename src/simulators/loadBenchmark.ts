...
function generateSyntheticPayload(index: number) {
  const paymentId = `pay_bench_${Date.now()}_${index}_${crypto.randomBytes(3).toString('hex')}`;
  const scenarios = [
...
async function sendWorker(queue: number[]): Promise<void> {
